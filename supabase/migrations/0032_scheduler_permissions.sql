-- ============================================================================
-- 0032: who creates and edits shorts / long videos
--
--   * Create shorts and long videos: master or scheduler (role id
--     'publisher'). Scripters no longer create; they write scripts.
--   * Title, type, platforms, caption, and dates of Auto shorts: master or
--     scheduler, at any stage.
--   * Fixed dates (changing, releasing, "Start queue from here") and
--     starting the queue: master only. A scheduler who gives an Auto short
--     a date gets "Just this one".
--
-- Staging first, then production.
-- ============================================================================

drop policy if exists "masters and scripters create shorts" on short_videos;
drop policy if exists "masters and schedulers create shorts" on short_videos;
create policy "masters and schedulers create shorts"
  ...
  with check (
    ...
    and (
      is_team_master(team_id)
      or (has_team_role(team_id, 'publisher') and pin_kind is distinct from 'anchor')
    )
  );

drop policy if exists "teammates can create projects" on long_video_projects;
drop policy if exists "masters and schedulers create projects" on long_video_projects;
create policy "masters and schedulers create projects"
  on long_video_projects for insert to authenticated
  with check (
    created_by = (select auth.uid())
    and team_id in (select my_team_ids())
    and (is_team_master(team_id) or has_team_role(team_id, 'publisher'))
    and (stage = 'ideate' or is_team_master(team_id))
  );

-- New fixed shorts: only a master's can default to starting the queue.
...
begin
  ...
  if new.planned_date is null then
    ...
    new.pin_kind := coalesce(
      new.pin_kind,
      case when is_team_master(new.team_id) and (active_queue_start(new.team_id)).id is null
           then 'anchor' else 'oneoff' end
    );
